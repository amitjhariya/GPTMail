window.onload = () => {
  console.log("opened linked");
  // Get the messaging inbox button element
    ...
    for (let mutation of mutationsList) {
      if (mutation.type === "childList") {
        for (let node of mutation.addedNodes) {
          if (
            node.classList &&
            node.classList.contains("msg-s-event-listitem__body")
          ) {
            captureMsg();
  ...
  const captureMsg = async () => {
    const sendBTn = document.querySelector(".msg-form__send-button");
    const button = document.createElement("div");
    button.innerHTML = `
    <button id="gptHandler"  style="cursor:pointer; background-color: #1b61d1;border: none;display: flex;justify-content: center;color: white;align-items: center;padding: 5px 20px 7px 10px;margin: 0 10px;border-radius: 25px;"> GPT<svg xmlns="http://www.w3.org/2000/svg"  class="gpt-svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="16" height="16" viewBox="0 0 16 16">
  ...
  `;
    ...
    sendBTn.parentNode.parentNode.appendChild(button);
  ...
  // Create a new MutationObserver and start observing the document for changes
